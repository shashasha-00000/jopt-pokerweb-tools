...
function classifyTask_(item) {
  ...
  var tournamentTags = inferTournamentTags_([
    item.cleanThreadRootText,
    item.cleanSourceText,
    latestText
  ].join('\n'));
...
function inferTournamentTags_(text) {
  var source = normalizeWhitespace_(text);
  var candidates = [];
  var tags = [];
  var bracketPattern = /【([^】]+)】/gu;
  var bracketMatch;
  while ((bracketMatch = bracketPattern.exec(source)) !== null) {
    candidates.push(bracketMatch[1]);
  }
  source.split('\n').forEach(function(line) {
    if (hasTournamentNameSignal_(line)) candidates.push(line);
  });
  candidates.forEach(function(candidate) {
    var tag = normalizeTournamentNameTag_(candidate);
    if (tag && tags.indexOf(tag) === -1) tags.push(tag);
  });
  return tags.join(', ');
}

function hasTournamentNameSignal_(value) {
  return /JOPT|SPADIE|戦国|SENGOKU|MASTER|U[\s-]?30|POKER|ポーカー|TOUR|ツアー|CUP|カップ|FESTIVAL|フェスティバル|CHAMPIONSHIP|選手権|SERIES|シリーズ/iu.test(value || '');
}

function normalizeTournamentNameTag_(value) {
  var raw = normalizeWhitespace_(value).replace(/^【|】$/gu, '');
  if (/U[\s-]?30/iu.test(raw)) return 'U-30';
  if (/\bMASTER\b/iu.test(raw)) return 'master';
  if (/戦国|SENGOKU/iu.test(raw)) return '戦国';
  if (/\bJOPT\b/iu.test(raw)) return buildBrandLocationTag_('JOPT', raw);
  if (/\bSPADIE\b/iu.test(raw)) return buildBrandLocationTag_('SPADIE', raw);

  var tag = raw
    .replace(/^【|】$/gu, '')
    .split(/事前予約|店舗|工程表|受付|ドリンク|反映|掲載|について|確認|対応/u)[0]
    .replace(/\b20\d{2}\b/giu, ' ')
    .replace(/#\s*\d+[A-Z]?\b/giu, ' ')
    .replace(/\b(?:NO\.?|VOL\.?|SEASON)\s*\d+\b/giu, ' ')
    .replace(/第\s*\d+\s*回/gu, ' ')
    .replace(/\b\d+(?:ST|ND|RD|TH)\b/giu, ' ')
    .replace(/\bDAY\s*\d+[A-Z]?\b/giu, ' ')
    .replace(/[‐‑‒–—―-]+\s*(?:春|夏|秋|冬)の陣\s*[‐‑‒–—―-]*/gu, ' ')
    .replace(/[\[\]()（）]/gu, ' ')
    .replace(/\s+/g, ' ')
    .trim();

  if (!tag || !hasTournamentNameSignal_(tag)) return '';
  tag = tag.replace(/[A-Za-z]+/g, function(word) { return word.toUpperCase(); });
  return tag.length <= 40 ? tag : '';
}

function buildBrandLocationTag_(brand, value) {
  var source = String(value || '').toUpperCase();
  if (brand === 'JOPT' && /GRAND\s*FINAL/u.test(source)) return 'JOPT TOKYO';
  var locations = [
    ['TOKYO', /TOKYO|東京/u],
    ['SAPPORO', /SAPPORO|札幌/u],
    ['OSAKA', /OSAKA|大阪/u],
    ['FUKUOKA', /FUKUOKA|福岡/u],
    ['NAGOYA', /NAGOYA|名古屋/u],
    ['OKINAWA', /OKINAWA|沖縄/u],
    ['YOKOHAMA', /YOKOHAMA|横浜/u],
    ['SENDAI', /SENDAI|仙台/u],
    ['KYOTO', /KYOTO|京都/u],
    ['KOBE', /KOBE|神戸/u],
    ['HIROSHIMA', /HIROSHIMA|広島/u],
    ['TAKAMATSU', /TAKAMATSU|高松/u],
    ['HOKKAIDO', /HOKKAIDO|北海道/u]
  ];
  for (var index = 0; index < locations.length; index += 1) {
    if (locations[index][1].test(source)) return brand + ' ' + locations[index][0];
  }

  var stripped = source
    .replace(new RegExp('\\b' + brand + '\\b', 'g'), ' ')
    .replace(/\b20\d{2}\b/gu, ' ')
    .replace(/#\s*\d+[A-Z]?\b/gu, ' ')
    .replace(/\b(?:GRAND|FINAL|POKER|TOUR|FESTIVAL|CHAMPIONSHIP|SERIES|OPEN)\b/gu, ' ')
    .replace(/\b(?:NO\.?|VOL\.?|SEASON|DAY)\s*\d+[A-Z]?\b/gu, ' ')
    .replace(/\b\d+(?:ST|ND|RD|TH)\b/gu, ' ')
    .replace(/[^A-Z]+/gu, ' ')
    .trim();
  var location = stripped.split(/\s+/)[0] || '';
  return location && location.length <= 20 ? brand + ' ' + location : brand;
}
