...
function markDuplicatePreReservationApplications() {
  ...
  const resolution = preResMailerResolveDuplicateApplications_(source.rows);
  if (resolution.multipleConfirmedGroups.length > 0) {
    const details = resolution.multipleConfirmedGroups.slice(0, 10).map(group =>
      '- ' + group.confirmedRows.map(row =>
        '元シート' + row.sourceRow + '行 / ' + (row.playerName || '名前なし') + ' / ' + (row.gameId || 'Game IDなし')
      ).join(' ・ ')
    );
    SpreadsheetApp.getUi().alert(
      '重複申請の整理を停止しました。\n\n' +
      '同一申請グループに「決済確認」済みが2行以上あります。\n' +
      'このグループは自動で重複判定できません。\n\n' +
      details.join('\n')
    );
    return;
  ...
  const duplicateRows = resolution.automaticDuplicateRows;
  const rowsToMark = duplicateRows.concat(
    resolution.explicitDuplicateRows.filter(row => !/重複/i.test(preResMailerText_(row.manualAction)))
  );

  let newlyMarked = 0;
  if (source.rowCount > 0 && rowsToMark.length > 0) {
  ...
  SpreadsheetApp.getUi().alert(
    ...
    '決済確認済みを優先保留: ' + resolution.paymentConfirmedWinners.length + '件\n' +
    '自動で重複として扱う申請: ' + duplicateRows.length + '件\n' +
    '管理列の「重複」を反映: ' + (rowsToMark.length - duplicateRows.length) + '件\n' +
...
function syncLivePocketPaymentConfirmations() {
  ...
  const duplicateResolution = preResMailerResolveDuplicateApplications_(source.rows);
  const activeCandidateRows = {};
  const activeLatestRows = {};
  const duplicateRows = {};
  const conflictingConfirmedRows = {};
  duplicateResolution.candidateRows.forEach(row => {
    activeCandidateRows[row.sourceRow] = true;
  });
  duplicateResolution.activeRows.forEach(row => {
    activeLatestRows[row.sourceRow] = true;
  });
  duplicateResolution.explicitDuplicateRows
    .concat(duplicateResolution.automaticDuplicateRows)
    .forEach(row => {
      duplicateRows[row.sourceRow] = true;
    });
  duplicateResolution.multipleConfirmedGroups.forEach(group => {
    group.rows.forEach(row => {
      conflictingConfirmedRows[row.sourceRow] = true;
    });
  });
...
function preResMailerClassifyLivePocketPayment_(
  matches,
  activeCandidateRows,
  activeLatestRows,
  duplicateRows,
  ...
  if (!latestMatch) {
    const hasConfirmedConflict = matches.some(row => conflictingConfirmedRows[row.sourceRow]);
    if (hasConfirmedConflict) {
      return { status: '決済確認済み重複・要確認', category: 'needs_review', row: matches[0] };
    }
    const isOldDuplicate = matches.some(row => duplicateRows[row.sourceRow]) || matches.some(row =>
      activeCandidateRows[row.sourceRow] && !activeLatestRows[row.sourceRow]
    );
    return {
      status: isOldDuplicate ? '重複申請・未反映' : '対象外・要確認',
...
function preResMailerBuildReport_(mailTypeFilter, reportLabel) {
  ...
  latestRows.forEach(row => {
    ...
    if (mailType === 'coin_payment' || mailType === 'livepocket_payment') {
  ...
  preResMailerWritePaymentDeadlines_(ctx.sourceSheet, ctx.sourceMap, paymentDeadlineRows);
...
function preResMailerFindSourceColumnMap_(sheet) {
  ...
  for (let rowIndex = 0; rowIndex < values.length; rowIndex++) {
    ...
    map.participantGameIdCols = preResMailerFindParticipantGameIdColumns_(headers);
    if (map.gameIdCol && map.participantGameIdCols.indexOf(map.gameIdCol) < 0) {
      map.participantGameIdCols.unshift(map.gameIdCol);
    }
...
function preResMailerFindParticipantGameIdColumns_(headers) {
  const columns = [];
  headers.forEach((header, index) => {
    const text = preResMailerText_(header);
    const startsWithGameId = /^(?:Game\s*ID|GameID|ゲーム\s*ID)(?:$|【|（|\(|\s)/i.test(text);
    const participantRoleThenGameId = /(?:代表者|メンバー|選手|プレイヤー).*(?:Game\s*ID|GameID|ゲーム\s*ID)$/i.test(text);
    if (startsWithGameId || participantRoleThenGameId) columns.push(index + 1);
  });
  return columns;
}
...
function preResMailerReadSourceRowsWithoutUpdates_(sheet, map, deadlineBaseTime) {
  const startRow = map.headerRow + 1;
  ...
  const calculationBaseTime = deadlineBaseTime instanceof Date
    ? new Date(deadlineBaseTime.getTime())
    : new Date();
  return {
    startRow: startRow,
    rowCount: rowCount,
    ...
    rows: values.map((row, index) =>
      preResMailerParseSourceRow_(row, startRow + index, map, calculationBaseTime)
    )
  };
}

function preResMailerWritePaymentDeadlines_(sheet, map, rows) {
  if (!map.paymentDeadlineCol || !rows.length) return;

  const ranges = [];
  const seenRows = {};
  let deadline = null;
  rows.forEach(row => {
    if (seenRows[row.sourceRow] || !row.computedDeadline) return;
    const existing = row.paymentDeadlineRaw;
    if (existing !== '' && existing != null) return;
    seenRows[row.sourceRow] = true;
    deadline = deadline || row.computedDeadline;
    ranges.push(sheet.getRange(row.sourceRow, map.paymentDeadlineCol).getA1Notation());
  });
...
function preResMailerParseSourceRow_(values, sourceRow, map, deadlineBaseTime) {
  const paymentDeadlineRaw = map.paymentDeadlineCol ? values[map.paymentDeadlineCol - 1] : '';
  const explicitDeadline = preResMailerValidDate_(paymentDeadlineRaw);
  const timestamp = map.timestampCol ? values[map.timestampCol - 1] : '';
  const computedDeadline = explicitDeadline ? null : preResMailerCalculateBusinessDeadline_(deadlineBaseTime);
  ...
  return {
    ...
    paymentDeadlineRaw: paymentDeadlineRaw,
...
function preResMailerKeepLatestRows_(rows) {
  const resolution = preResMailerResolveDuplicateApplications_(rows);
  return resolution.activeRows
    .filter(row => !preResMailerHasOverlapConflict_(row, resolution.candidateRows));
}
...
function preResMailerGroupRowsByIdentity_(rows) {
  ...
  rows.forEach((row, index) => {
    ...
    if (applicantKey && byApplicantSet[applicantKey] != null) union(index, byApplicantSet[applicantKey]);
    if (emailKey) byEmail[emailKey] = index;
    if (gameKey) byGameId[gameKey] = index;
    if (applicantKey) byApplicantSet[applicantKey] = index;
  });
...
function preResMailerResolveDuplicateApplications_(rows) {
  const identityRows = rows.filter(row => row.email || row.gameId);
  const explicitDuplicateRows = identityRows.filter(preResMailerHasExplicitDuplicateMarker_);
  const candidateRows = identityRows.filter(row =>
    !preResMailerHasExplicitDuplicateMarker_(row) &&
    !preResMailerIsManualSkip_(row.manualAction)
  );
  const groups = preResMailerGroupRowsByIdentity_(candidateRows);
  const activeRows = [];
  const automaticDuplicateRows = [];
  const multipleConfirmedGroups = [];
  const paymentConfirmedWinners = [];

  groups.forEach(group => {
    const confirmedRows = group.rows.filter(row => row.paymentConfirmed);
    group.confirmedRows = confirmedRows;
    if (confirmedRows.length > 1) {
      group.active = null;
      multipleConfirmedGroups.push(group);
      return;
    }

    const active = confirmedRows.length === 1 ? confirmedRows[0] : group.latest;
    group.active = active;
    activeRows.push(active);
    if (confirmedRows.length === 1 && group.rows.length > 1) paymentConfirmedWinners.push(active);
    group.rows.forEach(row => {
      if (row.sourceRow !== active.sourceRow) automaticDuplicateRows.push(row);
    });
  });

  return {
    identityRows: identityRows,
    explicitDuplicateRows: explicitDuplicateRows,
    candidateRows: candidateRows,
    groups: groups,
    activeRows: activeRows,
    automaticDuplicateRows: automaticDuplicateRows,
    multipleConfirmedGroups: multipleConfirmedGroups,
    paymentConfirmedWinners: paymentConfirmedWinners
  };
}
...
function preResMailerHasDuplicateMarker_(row) {
  if (preResMailerHasExplicitDuplicateMarker_(row)) return true;
  return preResMailerIsManualSkip_(row.manualAction);
}

function preResMailerHasExplicitDuplicateMarker_(row) {
  if (/重複/i.test(preResMailerText_(row.manualAction))) return true;
  return [
...
function preResMailerHasInvalidManagementMarker_(row) {
  return [
    ...
    return text && normalized !== 'TRUE' && normalized !== 'FALSE' && !/重複/i.test(text);
  });
}

function preResMailerCollectParticipantGameIds_(values, map) {
  const ids = [];
  const columns = map.participantGameIdCols && map.participantGameIdCols.length
    ? map.participantGameIdCols
    : (map.gameIdCol ? [map.gameIdCol] : []);
  columns.forEach(column => {
    const normalized = preResMailerNormalizeGameId_(values[column - 1]);
