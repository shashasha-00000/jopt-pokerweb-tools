...
test7777\t2026/07/02\t13:00\t80000\t1000\t1\t80000\t0\t3\t-74998\t0\t0\t【SPADIE TOKYO 42nd】Main Event / -2026.08.31`;
  ...
  function parseInput(raw) {
    ...
    if (lines.length < 2) throw new Error("Input must contain header and one data row.");
    ...
    if (lines.length > 2) throw new Error("POC accepts exactly one tournament row.");
    ...
    if (!name || !date || !time) throw new Error("大会名 / 日付 / 開始時間 are required.");
  ...
  function renderPreview() {
    clearLog();
    const t = parseInput($("#pw-bg-poc-input").value);
    log("PREVIEW");
  ...
  async function runCreate() {
    clearLog();
    const t = parseInput($("#pw-bg-poc-input").value);
    const ok = confirm(`Create test tournament?\n\n${t.name}\n${t.date} ${t.time}\n\nThis writes to PokerWeb.`);
    if (!ok) return;
  ...
  function addPanel() {
    ...
    panel.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;margin-bottom:8px;">
        <strong>PW Background Create API POC</strong>
      ...
      <div style="margin-top:6px;color:#f6d365;">Test-only. Not listed in scripts.json. CREATE button writes to PokerWeb.</div>
    `;

    document.body.appendChild(panel);
    $("#pw-bg-poc-input").value = DEFAULT_INPUT;
