 ...
 * 元シート「フォームの回答 5」は、送信成功時の A/D/E/F 列チェックと
 * Q列の支払期日、R列の手動指示以外は変更しない。
 */

const RESERVATION_MAIL_CONFIG = {
  sourceSheetName: 'フォームの回答 5',
  headerRow: 13,
  dataStartRow: 14,
  ...
  columns: {
    ...
    paymentReminderSent: 5, // E: チェック済み = 支払再案内を送信済み
    cancelMailSent: 6,    // F: チェック済み = キャンセルメール送信済み
    email: 8,             // H
    gameId: 9,            // I
    playerName: 10,       // J
    paymentMethod: 11,    // K
    paymentDeadline: 17,  // Q: 支払期日
    manualAction: 18      // R: cancel / キャンセル
  },
  ...
  eventName: 'JOPT 2026 Tokyo #02 / PLO Heads-up',
  livePocketUrl: 'https://livepocket.jp/e/oe493',
...
function reservationMailReportOnOpen() {
  SpreadsheetApp.getUi()
    ...
    .addSeparator()
    .addItem('支払最終案内メール一括作成', 'createPaymentReminderReport')
...
function createPaymentReminderReport() {
  reservationBuildReport_({
    sheetName: 'REPORT_PAYMENT_FINAL_NOTICE',
...
function markOverdueRowsAsCancelCandidates() {
  ...
  SpreadsheetApp.getUi().alert(
    ...
    cellsToMark.length + '件\n\nR列を確認し、送信しない方は「キャンセル」を削除してください。'
  );
}
...
function setupReservationDeadlineAndColors() {
  ...
  const c = RESERVATION_MAIL_CONFIG.columns;
  sheet.getRange(RESERVATION_MAIL_CONFIG.headerRow, c.paymentDeadline).setValue('支払期日');
  ...
  const rules = [
    SpreadsheetApp.newConditionalFormatRule()
      .whenFormulaSatisfied('=$F' + start + '=TRUE')
      .setBackground('#d9d9d9')
      .setRanges([target])
      .build(),
    SpreadsheetApp.newConditionalFormatRule()
      .whenFormulaSatisfied('=OR(LOWER($R' + start + ')="cancel",$R' + start + '="キャンセル")')
    ...
    SpreadsheetApp.newConditionalFormatRule()
      ...
      .build(),
    SpreadsheetApp.newConditionalFormatRule()
      .whenFormulaSatisfied('=AND($G' + start + '<>"",$B' + start + '=FALSE,$Q' + start + '<TODAY())')
  ...
  SpreadsheetApp.getUi().alert('Q列の支払期日と自動色分けを設定しました。');
}
...
function reservationUpdateSourceAfterSend_(source, sourceRow, mailType) {
  ...
  if (mailType === '支払予定確認・リマインド') {
    source.getRange(sourceRow, c.paymentReminderSent).setValue(true);
  }
...
function reservationParseSourceRow_(values, sourceRow, timestampColumn) {
  ...
  return {
    ...
    paymentReminderSent: reservationIsChecked_(values[c.paymentReminderSent - 1]),
...
function reservationMakeCoinPaymentInvite_(row) {
  return {
    subject: '??????????' + RESERVATION_MAIL_CONFIG.eventName,
    body: `${row.playerName} ?

????????????
????????????????????????????

???? ?${RESERVATION_MAIL_CONFIG.eventName}? ???????????????????????
????????????????????

?????
Poker Web Coin ???JOPT?????? 30,000 coin???????????????
?????????????GameID?????????
...
??????????${RESERVATION_MAIL_CONFIG.livePocketUrl}
????30,000?
????????${row.deadlineWithTime}
?????????????????????????????????????
????????????????????????????????????????????????
