 ...
 * - Duplicate applications are merged when representative email or representative GameID overlaps; the latest timestamp wins.
 ...
 * - X列 メール指示 = 重複申込 means the row is excluded from every normal mail report.
 * - Test rows are excluded from every mail report.
 */
...
function reservation3on3MailReportOnOpen() {
  ...
  const menu = ui.createMenu('3on3事前予約メールREPORT')
    ...
    .addItem('重複スキャンREPORT：全プレイヤーGameID確認', 'create3on3PlayerDuplicateScanReport')
    .addItem('重複旧行をX列「重複申込」にする', 'mark3on3OlderDuplicatePlayerRows')
    .addItem('テスト行をX列「テスト」にする', 'mark3on3TestRows')
    .addSeparator()
    ...
    .addItem('選択範囲キャンセル通知', 'create3on3JpSelectedCancelReport')
    .addSeparator()
...
function create3on3JpSelectedCancelReport() {
  reservation3on3RunReport_('jp', 'cancel_selected', {
    sourceRows: reservation3on3SelectedSourceRows_()
  });
}
...
function create3on3PlayerDuplicateScanReport() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const source = ss.getSheetByName(R3ON3.sheets.jp.name);
  if (!source) throw new Error('元シート「' + R3ON3.sheets.jp.name + '」が見つかりません。');

  const duplicateGroups = reservation3on3CollectPlayerDuplicateGroups_(source);
  ...
  duplicateGroups.forEach(group => {
    group.items.forEach(item => {
      output.push([
        ...
        item.isLatest ? '最新申込として残す' : '旧申込候補：X列「重複申込」対象'
      ]);
    });
  });

  const report = reservation3on3GetOrCreateReportSheet_(ss, 'REPORT_3ON3_PLAYER_DUPLICATES');
  report.clear();
  report.getRange(1, 1, output.length, output[0].length).setValues(output);
  report.getRange(1, 1, 1, output[0].length).setFontWeight('bold').setBackground('#fce5cd');
  report.setFrozenRows(1);
  report.autoResizeColumns(1, output[0].length);
  report.activate();

  SpreadsheetApp.getUi().alert(
    '全プレイヤーGameIDの重複スキャンを作成しました。\n重複行: ' + Math.max(output.length - 1, 0) + '件'
  );
}

function mark3on3OlderDuplicatePlayerRows() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const source = ss.getSheetByName(R3ON3.sheets.jp.name);
  if (!source) throw new Error('元シート「' + R3ON3.sheets.jp.name + '」が見つかりません。');
  ...
  duplicateGroups.forEach(group => {
    group.items.forEach(item => {
      if (!item.isLatest && !item.cancelChecked) oldRows[item.sourceRow] = true;
    });
  });

  const rowNumbers = Object.keys(oldRows).map(Number).sort((a, b) => a - b);
  rowNumbers.forEach(rowNumber => {
    source.getRange(rowNumber, R3ON3.manualInstructionColumn).setValue('重複申込');
  });

  create3on3PlayerDuplicateScanReport();
  SpreadsheetApp.getUi().alert(
    '重複旧行をX列「重複申込」にしました。\n対象: ' + rowNumbers.length + '行'
  );
}

function mark3on3TestRows() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const source = ss.getSheetByName(R3ON3.sheets.jp.name);
  if (!source) throw new Error('元シート「' + R3ON3.sheets.jp.name + '」が見つかりません。');
  ...
  const rows = reservation3on3ReadLatestRows_(source, 'jp');
  const targets = rows
    .filter(row => row.isTestRow)
    .map(row => row.sourceRow)
    .sort((a, b) => a - b);

  targets.forEach(rowNumber => {
    source.getRange(rowNumber, R3ON3.manualInstructionColumn).setValue('テスト');
  });
...
function reservation3on3CollectPlayerDuplicateGroups_(source) {
  const lastRow = source.getLastRow();
  if (lastRow < R3ON3.dataStartRow) return [];

  const readCols = Math.max(source.getLastColumn(), R3ON3.manualInstructionColumn);
  const values = source
    .getRange(R3ON3.dataStartRow, 1, lastRow - R3ON3.dataStartRow + 1, readCols)
    .getValues();
  const c = R3ON3.columns;
  const byGameId = {};

  values.forEach((row, index) => {
    const sourceRow = R3ON3.dataStartRow + index;
    const timestamp = row[c.timestamp - 1];
    const timestampDate = reservation3on3ValidDate_(timestamp);
    const cancelChecked = reservation3on3IsChecked_(row[c.cancel - 1]);
    const teamName = reservation3on3Text_(row[c.teamName - 1]);
    const repEmail = reservation3on3Text_(row[c.email - 1]);
    const manualInstruction = reservation3on3Text_(row[R3ON3.manualInstructionColumn - 1]);
    const entries = [
      {
        role: '代表者',
        name: reservation3on3Text_(row[c.playerName - 1]),
        gameId: reservation3on3NormalizeGameId_(row[c.gameId - 1])
      },
      {
        role: 'プレイヤー②',
        name: reservation3on3Text_(row[c.player2Name - 1]),
        gameId: reservation3on3NormalizeGameId_(row[c.player2GameId - 1])
      },
      {
        role: 'プレイヤー③',
        name: reservation3on3Text_(row[c.player3Name - 1]),
        gameId: reservation3on3NormalizeGameId_(row[c.player3GameId - 1])
      }
    ];

    entries.forEach(entry => {
      if (!entry.gameId) return;
      if (!byGameId[entry.gameId]) byGameId[entry.gameId] = [];
      byGameId[entry.gameId].push({
        gameId: entry.gameId,
        sourceRow,
        role: entry.role,
        name: entry.name,
  ...
  return Object.keys(byGameId)
    .sort()
    .map(gameId => {
      const items = byGameId[gameId].filter((item, index, all) =>
        all.findIndex(other => other.sourceRow === item.sourceRow && other.role === item.role) === index
      );
      const uniqueRows = {};
      items.forEach(item => {
        uniqueRows[item.sourceRow] = true;
      });
      if (Object.keys(uniqueRows).length <= 1) return null;

      const latest = items.reduce((winner, item) =>
        reservation3on3CompareDuplicateItemFreshness_(item, winner) > 0 ? item : winner
      );
      items.forEach(item => {
        item.isLatest = item.sourceRow === latest.sourceRow;
      });

      return {
        gameId,
        items: items.sort((a, b) => a.sourceRow - b.sourceRow || a.role.localeCompare(b.role))
      };
    })
    .filter(Boolean);
}

function reservation3on3CompareDuplicateItemFreshness_(a, b) {
...
function reservation3on3RunReport_(lang, type, options) {
  const definition = reservation3on3Definition_(lang, type);
  if (options && options.sourceRows) definition.sourceRows = options.sourceRows;
  ...
  const defs = {
    ...
    final_notice: {
      ...
      matches: row => reservation3on3NormalMailTarget_(row) &&
        ...
        reservation3on3IsSameDate_(row.deadlineDate, reservation3on3Today_()),
    ...
    cancel_selected: {
      sheetName: 'REPORT_3ON3_' + lang.toUpperCase() + '_CANCEL_SELECTED',
      mailType: labelPrefix + ' キャンセル通知',
      mark: 'cancel',
      matches: row => row.isLatest && row.cancelChecked && !row.cancelNoticeSent &&
        !row.isDuplicateRow && !row.isTestRow,
      makeMail: row => reservation3on3Mail_(lang, 'cancel', row)
    }
  };
...
function reservation3on3NormalMailTarget_(row) {
  return row.isLatest && !row.cancelChecked && !row.isDuplicateRow && !row.isTestRow;
}
...
function reservation3on3SetupOneSheet_(sheet) {
  ...
  const validation = SpreadsheetApp.newDataValidation()
    .requireValueInList(['', 'ticket要確認', 'ticket確認メール済', '重複申込', 'テスト', 'キャンセル通知済'], true)
...
function reservation3on3BuildReport_(definition) {
  ...
  rows.forEach(row => {
    if (definition.sourceRows && !definition.sourceRows[row.sourceRow]) return;
...
function reservation3on3SelectedSourceRows_() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getActiveSheet();
  if (!sheet || sheet.getName() !== R3ON3.sheets.jp.name) {
    throw new Error('元シート「' + R3ON3.sheets.jp.name + '」で処理したい行を選択してください。');
  }

  const range = sheet.getActiveRange();
  if (!range) throw new Error('処理したい行を選択してください。');

  const rows = {};
  const firstRow = Math.max(range.getRow(), R3ON3.dataStartRow);
  const lastRow = range.getLastRow();
  for (let row = firstRow; row <= lastRow; row++) {
    rows[row] = true;
  }
  if (Object.keys(rows).length === 0) {
    throw new Error('データ行を選択してください。');
  }
  return rows;
}
...
function reservation3on3ReadLatestRows_(sheet, lang) {
  ...
  reservation3on3AssignDedupeGroups_(rows);
...
function reservation3on3AssignDedupeGroups_(rows) {
  ...
  rows.forEach((row, index) => {
    const keys = reservation3on3IdentityKeys_(row);
    keys.forEach(key => {
      if (keyOwner[key] == null) {
        keyOwner[key] = index;
      } else {
        union(index, keyOwner[key]);
      }
    });
  });
  ...
  rows.forEach((row, index) => {
    const keys = reservation3on3IdentityKeys_(row);
    row.identityKeys = keys.join(',');
    row.dedupeKey = keys.length ? 'grp:' + find(index) : 'row:' + row.sourceRow;
  });
}

function reservation3on3IdentityKeys_(row) {
  const keys = [];
  if (row.email) keys.push('mail:' + row.email.toLowerCase());
  if (row.gameId) keys.push('gid:' + row.gameId);
  return keys;
}
...
function reservation3on3ParseSourceRow_(values, sourceRow, lang) {
  ...
  const teamName = reservation3on3Text_(values[c.teamName - 1]);
  const player2Name = reservation3on3Text_(values[c.player2Name - 1]);
  const player2GameId = reservation3on3NormalizeGameId_(values[c.player2GameId - 1]);
  const player3Name = reservation3on3Text_(values[c.player3Name - 1]);
  const player3GameId = reservation3on3NormalizeGameId_(values[c.player3GameId - 1]);
  ...
  return {
    ...
    isDuplicateRow: reservation3on3DuplicateInstruction_(manualInstruction),
    isTestRow: reservation3on3IsTestRow_({
      playerName,
      email,
      gameId,
      teamName,
...
function reservation3on3DuplicateInstruction_(value) {
  return reservation3on3Text_(value) === '重複申込';
}

function reservation3on3IsTestRow_(row) {
  const text = [
    row.playerName,
    row.email,
    row.gameId,
...
function reservation3on3IsSameDate_(a, b) {
  if (!a || !b) return false;
